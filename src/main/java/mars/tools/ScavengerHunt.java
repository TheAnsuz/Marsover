...
public class ScavengerHunt implements Observer, MarsTool {
    ...
    private class ScavengerHuntDisplay extends JPanel {
        ...
        public void paintComponent(Graphics g) {
            ...
            if (!GameOn) // Make sure game is ready before continuing
            {
                ...
                g2
                        .drawString(" ScavengerHunt not yet initialized by MIPS administrator program.",
                                100, 200);
                return;
            }
            ...
            for (int i = 0; i < NUM_PLAYERS; i++) {
                ...
                g2
                        .drawString(" " + pd[i].getEnergy(), width - 40, 50 + (i * 15));  // Player's energy on scoreboard
                ...
                } else // player either has not finished or is just now finishing
                {
                    ...
                    if (visCount == NUM_LOCATIONS) // player has visited every location -- finished!
                    {
                        pd[i].setFinished();
                        pd[i]
                                .setFinishTime(System.currentTimeMillis() - startTime);
                    ...
                    } else // player has not yet visited every location
                    {
                        ...
                        for (int j = 0; j < NUM_LOCATIONS; j++) {
                            ...
                            {
                                g2
                                        .fillRect((width - 120) + (j * 10), 42 + (i * 15), 10, 8);
                            }
                        }
                    }
    ...
    public void update(Observable o, Object arg) {
        ...
        } else if (isWrite && address == ADDR_PLAYER_ID) // if the data written will change the PlayerID, authenticate the write
        {
            ...
            if (toolGetWord(ADDR_AUTHENTICATION) == authenticationValue) // Compare to "client's" version of the authenticationValue
            {
                ...
                System.out
                        .println("ScavengerHunt.update(): Invalid write of player ID! \nPlayer "
                                + playerID + " tried to write.  Expected:   "
                                + Binary.intToHexString(authenticationValue)
                                + ", got:  " + Binary
                                        .intToHexString(toolGetWord(ADDR_AUTHENTICATION)) + "\n");
            }
        } else if (isWrite
                ...
                && data != 0) //  Player wrote data to his/her assigned MoveReady location
        {
         ...
         System.out.println(" ******** ScavengerHunt.update: Player " + playerID + " requests move to (" +
                    ...
                    || toolReadPlayerData(playerID, OFFSET_MOVE_TO_Y) > GRAPHIC_HEIGHT) // Out of bounds. Player not allowed to move
            {
                ...
                //                                    "(This msg. in ScavengerHunt.update()" );
                System.out
                        .println("Player " + playerID + " can't move -- out of bounds.");
                return;
            }
            ...
            if (Math.sqrt(
                    ...
                    + Math
                            .pow(toolReadPlayerData(playerID, OFFSET_WHERE_AM_I_Y)
                ...
                toolWritePlayerData(playerID, OFFSET_WHERE_AM_I_Y,
                        toolReadPlayerData(playerID, OFFSET_MOVE_TO_Y));
                pd[playerID]
                        .setWhereAmI(toolReadPlayerData(playerID, OFFSET_WHERE_AM_I_X),
                ...
                pd[playerID]
                        .setEnergy(toolReadPlayerData(playerID, OFFSET_ENERGY));
                ...
                System.out
                        .println("Player " + playerID + " can't move -- exceeded max. movement.");
                ...
                && data != 0) //  Player wrote data to his/her assigned TaskComplete location
        {
            ...
            for (int i = 1; i < SIZE_OF_TASK; i++) {
                ...
                if (prevData > currentData) {
                    ...
                    System.out
                            .println("Whoops! Player has NOT completed task correctly");
                    return;
                }
                ...
                && address == (ADDR_BASE + (playerID * MEM_PER_PLAYER) + OFFSET_PLAYER_COLOR)) //  Player wrote data to his/her assigned PlayerColor location
        {
            ...
            pd[playerID]
                    .setColor(toolReadPlayerData(playerID, OFFSET_PLAYER_COLOR));
        } // TBD TBD TBD
        // TBD TBD TBD
        // TBD TBD TBD
    ...
    private void toolSetWord(int address, int data) {
        ...
        if (KENVDEBUG) {
            System.out
                    .println("   ScavengerHunt.toolSetWord: Setting MIPS Memory["
                            + Binary.intToHexString(address) + "] to " + Binary
                            .intToHexString(data) + " = " + data);
        }
        SetWordCounter++;
        ...
        try {
            ...
            System.out
                    .println("ScavengerHunt.toolSetWord: deliberate exit on AEE exception.");
            System.out.println("     SetWordCounter = " + SetWordCounter);
            System.out.println("     address = " + Binary
                    .intToHexString(address));
            System.out.println("     data = " + data);
            System.exit(0);
        } catch (Exception e) {
            System.out
                    .println("ScavengerHunt.toolSetWord: deliberate exit on " + e
        ...
        try {
         ...
         System.out.println("ScavengerHunt.toolGetWord: Mem[" +
          ...
          Binary.intToHexString(returnValue) + " --- returning normally");
            ...
            System.out
                    .println("ScavengerHunt.toolGetWord: deliberate exit on AEE exception.");
            System.out.println("     SetWordCounter = " + SetWordCounter);
            System.out.println("     address = " + Binary
                    .intToHexString(address));
            System.exit(0);
            ...
            System.out
                    .println("ScavengerHunt.toolGetWord: deliberate exit on " + e
                            .getMessage() + " exception.");
            System.out.println("     SetWordCounter = " + SetWordCounter);
            System.out.println("     address = " + Binary
                    .intToHexString(address));
    ...
    private int toolReadPlayerData(int p, int offset) {
        ...
        if (KENVDEBUG) {
            System.out
                    .println("ScavengerHunt.toolReadPlayerData: called with player " + p
                            + ", offset = "
                            + Binary.intToHexString(offset) + " ---> address "
                            + Binary
                                    .intToHexString(ADDR_BASE + (p * MEM_PER_PLAYER) + offset)
                    );
        }
    ...
    private void toolWritePlayerData(int p, int offset, int data) {
        ...
        if (KENVDEBUG) {
            System.out
                    .println("ScavengerHunt.toolWritePlayerData: called with player " + p
        ...
        if (KENVDEBUG) {
            ...
            if (data != verifyData) {
                ...
                System.out
                        .println("      requested data to be written was " + Binary
                                .intToHexString(data));
                System.out.println("      actual data at that loc is " + Binary
                        .intToHexString(toolGetWord(address)));
                System.exit(0);
            } else
