        ...
        KeyboardFocusManager.getCurrentKeyboardFocusManager()
                .addKeyEventDispatcher(
                    ...
                    public boolean dispatchKeyEvent(KeyEvent e) {
                        if (JEditTextArea.this.isFocusOwner() && e.getKeyCode() == KeyEvent.VK_TAB && e
                                .getModifiers() == 0) {
                            processKeyEvent(e);
                            return true;
                        } else {
...
//       }
    ...
    public void updateScrollBars() {
        if (vertical != null && visibleLines != 0) {
            ...
            vertical
                    .setUnitIncrement(VERTICAL_SCROLLBAR_UNIT_INCREMENT_IN_LINES);
            ...
            lineNumbersVertical
                    .setValues(firstLine * height, visibleLines * height, 0, getLineCount() * height);
            lineNumbersVertical
                    .setUnitIncrement(VERTICAL_SCROLLBAR_UNIT_INCREMENT_IN_LINES * height);
...
// protected members
    ...
    class MouseWheelHandler implements MouseWheelListener {
        ...
        public void mouseWheelMoved(MouseWheelEvent e) {
            ...
            if (e.getWheelRotation() < 0) {
                setFirstLine(getFirstLine() - Math
                        .min(maxMotion, getFirstLine()));
            } else {
                setFirstLine(getFirstLine() + (Math.min(maxMotion, Math
                        .max(0, getLineCount() - (getFirstLine() + visibleLines)))));
            }
        }
    }
    ...
    public String getSyntaxSensitiveToolTipText(int x, int y) {
        ...
        ArrayList matches = getSyntaxSensitiveHelpAtLineOffset(line, this
                .xToOffset(line, x), true);
        if (matches == null) {
            return null;
        }
        ...
        for (int i = 0; i < matches.size(); i++) {
            ...
            result += ((i == 0) ? "" : "<br>") + "<tt>" + match
                    .getExamplePaddedToLength(length).replaceAll(" ", "&nbsp;") + "</tt>" + match
                    .getDescription();
        }
        return result + "</html>";
    }
    ...
    private ArrayList getSyntaxSensitiveHelpAtLineOffset(int line, int offset, boolean exact) {
        ...
        if (tokenMarker != null) {
            ...
            if (tokenAtOffset != null) {
                String tokenText = lineSegment.toString()
                        .substring(tokenOffset, tokenOffset + tokenAtOffset.length);
                if (exact) {
                    matches = tokenMarker
                            .getTokenExactMatchHelp(tokenAtOffset, tokenText);
                } else {
                    matches = tokenMarker.getTokenPrefixMatchHelp(lineSegment
                            .toString(), tokenList, tokenAtOffset, tokenText);
                }
            }
        }
        return matches;
    ...
    private void applySyntaxSensitiveHelp() {
        if (!mars.Globals.getSettings()
                .getBooleanSetting(mars.Settings.POPUP_INSTRUCTION_GUIDANCE)) {
            return;
        }
        int line = getCaretLine();
        int lineStart = getLineStartOffset(line);
        ...
        if (helpItems != null) {
            ...
            for (int i = 0; i < helpItems.size(); i++) {
                ...
                JMenuItem menuItem = new JMenuItem("<html><tt>" + item
                        .getExamplePaddedToLength(length)
                        .replaceAll(" ", "&nbsp;") + "</tt>" + item
                        .getDescription() + "</html>");
                if (item.getExact()) {
                    ...
                    menuItem.addActionListener(new PopupHelpActionListener(item
                            .getTokenText(), item.getExample()));
                }
    ...
    private class PopupHelpActionListener implements ActionListener {
        ...
        public void actionPerformed(ActionEvent e) {
            ...
            if (this.tokenText.length() >= this.text.length()) {
                ...
                overwriteSetSelectedText(this.text.substring(this.tokenText
                        .length()) + insert);
            }
        }
    }
        ...
        if ((evt.getKeyCode() == KeyEvent.VK_TAB || evt.getKeyCode() == KeyEvent.VK_ENTER)
            ...
            if (item.isEnabled()) {
                ...
                if (listeners.length > 0) {
                    listeners[0]
                            .actionPerformed(new ActionEvent(item, ActionEvent.ACTION_FIRST,
