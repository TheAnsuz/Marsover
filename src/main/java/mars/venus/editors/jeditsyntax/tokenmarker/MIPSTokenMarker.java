...
public class MIPSTokenMarker extends TokenMarker {
                     	...
                     	break;
                            ...
                            try {
                                validIdentifier = mars.assembler.TokenTypes
                                        .isValidIdentifier(new String(array, lastOffset, i1 - lastOffset - 1)
                                                .trim());
    ...
    public ArrayList getTokenExactMatchHelp(Token token, String tokenText) {
        ...
        if (token != null && token.id == Token.KEYWORD1) {
            ArrayList instrMatches = mars.Globals.instructionSet
                    .matchOperator(tokenText);
            if (!instrMatches.isEmpty()) {
                ...
                for (int i = 0; i < instrMatches.size(); i++) {
                    ...
                    if (mars.Globals.getSettings().getExtendedAssemblerEnabled() || inst instanceof BasicInstruction) {
                        matches.add(new PopupHelpItem(tokenText, inst
                                .getExampleFormat(), inst.getDescription()));
                        realMatches++;
                    }
                }
                if (realMatches == 0) {
                    matches
                            .add(new PopupHelpItem(tokenText, tokenText, "(is not a basic instruction)"));
                }
            }
        }
        if (token != null && token.id == Token.KEYWORD2) {
            ...
            if (dir != null) {
                matches = new ArrayList();
                matches.add(new PopupHelpItem(tokenText, dir.getName(), dir
                        .getDescription()));
            }
        }
        return matches;
    ...
    public ArrayList getTokenPrefixMatchHelp(String line, Token tokenList, Token token, String tokenText) {
        ...
        while (tokens.id != Token.END) {
            if (tokens.id == Token.KEYWORD1 || tokens.id == Token.KEYWORD2) {
                ...
                keywordTokenText = line
                        .substring(offset, offset + tokens.length);
        ...
        if (token != null && token.id == Token.NULL) {
            ...
            if (keywordTokenText == null && trimmedTokenText.length() > 0) {
                ...
                } else if (trimmedTokenText.length() >= mars.Globals
                        .getSettings().getEditorPopupPrefixLength()) {
    ...
    private ArrayList getTextFromDirectiveMatch(String tokenText, boolean exact) {
        ...
        if (directiveMatches != null) {
            ...
            for (int i = 0; i < directiveMatches.size(); i++) {
                ...
                matches
                        .add(new PopupHelpItem(tokenText, direct.getName(), direct
                                .getDescription(), exact));
            }
        }
        return matches;
    }
    ...
    private ArrayList getTextFromInstructionMatch(String tokenText, boolean exact) {
        ...
        for (int i = 0; i < matches.size(); i++) {
            ...
            if (mars.Globals.getSettings().getExtendedAssemblerEnabled() || inst instanceof BasicInstruction) {
                if (exact) {
                    results.add(new PopupHelpItem(tokenText, inst
                            .getExampleFormat(), inst.getDescription(), exact));
                } else {
        ...
        if (realMatches == 0) {
            if (exact) {
                results
                        .add(new PopupHelpItem(tokenText, tokenText, "(not a basic instruction)", exact));
            } else {
                return null;
            }
        } else {
            if (!exact) {
                ...
                while (mnemonicList.hasNext()) {
                    ...
                    results
                            .add(new PopupHelpItem(tokenText, mnemonic, info, exact));
                }
            }
        }
        return results;
    ...
    public static KeywordMap getKeywords() {
        if (cKeywords == null) {
            ...
            java.util.ArrayList instructionSet = mars.Globals.instructionSet
                    .getInstructionList();
            for (int i = 0; i < instructionSet.size(); i++) {
                cKeywords
                        .add(((mars.mips.instructions.Instruction) instructionSet
                                .get(i)).getName(), Token.KEYWORD1);
            }
            // add assembler directives
            java.util.ArrayList directiveSet = mars.assembler.Directives
                    .getDirectiveList();
            for (int i = 0; i < directiveSet.size(); i++) {
                cKeywords.add(((mars.assembler.Directives) directiveSet.get(i))
                        .getName(), Token.KEYWORD2);
            }
            // add integer register file
            mars.mips.hardware.Register[] registerFile = mars.mips.hardware.RegisterFile
                    .getRegisters();
            ...
            mars.mips.hardware.Register[] coprocessor1RegisterFile = mars.mips.hardware.Coprocessor1
                    .getRegisters();
            for (int i = 0; i < coprocessor1RegisterFile.length; i++) {
                cKeywords
                        .add(coprocessor1RegisterFile[i].getName(), Token.KEYWORD3);
            }
