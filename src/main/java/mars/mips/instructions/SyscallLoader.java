...
class SyscallLoader {
    ...
    void loadSyscalls() {
        ...
        ArrayList candidates = FilenameFinder.getFilenameList(this.getClass()
                .getClassLoader(),
        ...
        for (int i = 0; i < candidates.size(); i++) {
            ...
            if ((!file.equals(SYSCALL_INTERFACE))
                ...
                try {
                    ...
                    String syscallClassName = CLASS_PREFIX + file
                            .substring(0, file.indexOf(CLASS_EXTENSION) - 1);
                    ...
                    if (findSyscall(syscall.getNumber()) == null) {
                        ...
                        throw new Exception("Duplicate service number: " + syscall
                                .getNumber()
    ...
    private ArrayList processSyscallNumberOverrides(ArrayList syscallList) {
        ...
        for (int index = 0; index < overrides.size(); index++) {
            ...
            if (!match) {
                System.out
                        .println("Error: syscall name '" + override.getName()
        ...
        for (int i = 0; i < syscallList.size(); i++) {
            ...
            for (int j = i + 1; j < syscallList.size(); j++) {
                ...
                if (syscallA.getNumber() == syscallB.getNumber()) {
                    System.out
                            .println("Error: syscalls " + syscallA.getName() + " and "
                                    + syscallB.getName() + " are both assigned same number " + syscallA
                                    .getNumber());
                    duplicates = true;
                }
            }
